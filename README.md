...
<p align="center">
  ...
  <a href="https://pkg.go.dev/github.com/tehG30RG3/rosetta-digibyte?tab=overview"><img src="https://img.shields.io/badge/go.dev-reference-007d9c?logo=go&logoColor=white&style=shield" /></a>
</p>

<p align="center"><b>
ROSETTA-DIGIBYTE IS CONSIDERED <a href="https://en.wikipedia.org/wiki/Software_release_life_cycle#Alpha">ALPHA SOFTWARE</a>.
USE AT YOUR OWN RISK! NEITHER COINBASE NOR DIGIBYTE ASSUMES NO RESPONSIBILITY NOR LIABILITY IF THERE IS A BUG IN THIS IMPLEMENTATION.
</b></p>

## Overview
`rosetta-digibyte` provides an implementation of the Rosetta API for
DigiByte in Golang. If you haven't heard of the Rosetta API, you can find more
information [here](https://rosetta-api.org).

## Obligations
DigiByte is under an obligation to Coinbase for providing the reference implementation.
...
### Install
Running the following commands will create a Docker image called `rosetta-digibyte:latest`.

#### From GitHub
To download the pre-built Docker image from the latest release, run:
```text
curl -sSfL https://raw.githubusercontent.com/tehG30RG3/rosetta-digibyte/master/install.sh | sh -s
```
_Do not try to install rosetta-digibyte using GitHub Packages!_
...
a data directory at `<working directory>/digibyte-data` and the Rosetta API accessible
at port `8080`.

#### Mainnet:Online
```text
docker run -d --rm --ulimit "nofile=100000:100000" -v "$(pwd)/digibyte-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 12024:12024 rosetta-digibyte:latest
```
...
#### Testnet:Online
```text
docker run -d --rm --ulimit "nofile=100000:100000" -v "$(pwd)/digibyte-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 12026:12026 rosetta-digibyte:latest
```
...
## System Requirements
`rosetta-digibyte` has been tested on an [AWS c5.2xlarge instance](https://aws.amazon.com/ec2/instance-types/c5).
This instance type has 8 vCPU and 16 GB of RAM.

### Network Settings
To increase the load `rosetta-digibyte` can handle, it is recommended to tune your OS
...
sysctl -p (when done)
```
_We have not tested `rosetta-digibyte` with `net.ipv4.tcp_tw_recycle` and do not recommend
enabling it._
...
### Memory-Mapped Files
`rosetta-digibyte` uses [memory-mapped files](https://en.wikipedia.org/wiki/Memory-mapped_file) to
persist data in the `indexer`. As a result, you **must** run `rosetta-digibyte` on a 64-bit
...
## Architecture
`rosetta-digibyte` uses the `syncer`, `storage`, `parser`, and `server` package
from [`rosetta-sdk-go`](https://github.com/tehG30RG3/rosetta-sdk-go) instead
of a new DigiByte-specific implementation of packages of similar functionality. Below
...
### Optimizations
* Automatically prune digibyted while indexing blocks
...
To speed up indexing, `rosetta-digibyte` uses concurrent block processing
with a "wait free" design (using channels instead of sleeps to signal
which threads are unblocked). This allows `rosetta-digibyte` to fetch
...
To validate `rosetta-digibyte`, [install `rosetta-cli`](https://github.com/tehG30RG3/rosetta-cli#install)
