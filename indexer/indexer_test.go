...
import (
	...
	mocks "github.com/tehG30RG3/rosetta-digibyte/mocks/indexer"
...
func TestIndexer_Pruning(t *testing.T) {
	...
	cfg := &configuration.Configuration{
		...
		GenesisBlockIdentifier: digibyte.MainnetGenesisBlockIdentifier,
...
func TestIndexer_Transactions(t *testing.T) {
	...
	for i := int64(0); i <= 1000; i++ {
		...
		for j := 0; j < 5; j++ {
			...
			scriptPubKey := &digibyte.ScriptPubKey{
				ASM: coinIdentifier,
			}
	...
	go func() {
		for {
			...
			if currBlock.BlockIdentifier.Index == 1000 {
				...
				allCoins := []*types.Coin{}
				expectedPubKeys := []*digibyte.ScriptPubKey{}
